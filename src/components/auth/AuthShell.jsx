  ...
  return (
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-2 bg-neutral-light">
      ...
      >
        ...
        />
        <div
          className="pointer-events-none absolute -top-24 -right-16 h-72 w-72 rounded-full bg-white/10 blur-3xl animate-pulse"
          aria-hidden="true"
        />
        <div
          className="pointer-events-none absolute bottom-10 left-10 h-48 w-48 rounded-full bg-accent/20 blur-3xl"
          aria-hidden="true"
        />

        <div className="relative z-10">
          <a href="/" className="inline-flex items-center gap-3 rounded-xl bg-white/10 px-3 py-2 backdrop-blur-sm ring-1 ring-white/15 transition hover:bg-white/15">
            <img src={LogoImg} alt="Brovet" className="h-12 w-auto object-contain" />
          </a>
        </div>

        <div className="relative z-10 space-y-6 max-w-md fade-in">
          <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-2xs font-bold uppercase tracking-[0.16em] ring-1 ring-white/20">
            {isAdmin ? <Icons.Lock className="w-3.5 h-3.5" /> : <Icons.ShieldCheck className="w-3.5 h-3.5" />}
        ...
        <p className="relative z-10 text-2xs text-white/55">
      ...
      <section className="relative flex min-h-screen items-center justify-center px-4 py-10 sm:px-8">
        ...
        <div className="relative w-full max-w-md fade-in">
          ...
          <div className="rounded-2xl border border-neutral-border bg-white/95 p-6 sm:p-8 shadow-premium backdrop-blur-sm space-y-6">
            <div className="space-y-2 text-center sm:text-left">
              <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-2xs font-bold uppercase tracking-wider ${
                isAdmin
              ...
              <h2 className="text-2xl font-extrabold text-neutral-dark tracking-tight">{title}</h2>
              {subtitle && (
                <p className="text-sm text-neutral-muted leading-relaxed">{subtitle}</p>
              )}
            </div>
          ...
          {footer && (
            <div className="mt-5 text-center text-sm text-neutral-muted">
              {footer}
            </div>
          )}
        </div>
